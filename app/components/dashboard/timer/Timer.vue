<template>
  <div
    class="bg-white rounded-md p-4 shadow-md border-1 border-gray-200 flex flex-col justify-start items-center gap-4"
  >
    <State :state="currentTimerState" />

    <div class="flex flex-row justify-center items-start">
      <div class="flex flex-col justify-start items-center w-20">
        <p class="text-center text-7xl text-blue-500 font-semibold">
          {{ secondsToDate(elapsedTime) || "00:00:00" }}
        </p>
        <div
          class="flex flex-row justify-between items-center gap-22 font-medium text-gray-400"
        >
          <p>HRS</p>
          <p>MINS</p>
          <p>SECS</p>
        </div>
      </div>
    </div>

    <div class="info">
      <p class="text-center text-xl font-semibold text-gray-700">
        Design system updates
      </p>

      <div class="flex flex-row gap-1 text-lg text-gray-400">
        <p>Product design project</p>
        <p>/</p>
        <p>Hell hot</p>
      </div>
    </div>

    <hr class="w-6/12" />

    <div class="grid grid-cols-3 gap-6">
      <ControlButton :type="TimerButtonStates.PLAY" />
      <ControlButton :type="TimerButtonStates.PAUSE" />
      <ControlButton @click="stopTimer" :type="TimerButtonStates.STOP" />
    </div>
  </div>
</template>

<script setup lang="ts">
import { useTimerStore } from "~/store/useTimerStore.js";
import ControlButton from "./ControlButton.vue";
import State from "./State.vue";
import { useIntervalFn } from "@vueuse/core";

const store = useTimerStore();

const currentTimerState: Ref<TimerState> = ref(TimerState.NOT_STARTED);
const activeTimeSession: Ref<Partial<TimeSession> | undefined> = ref();

const timerPause: Ref<Function | undefined> = ref();

const elapsedTime: Ref<number> = ref(0);

function startTimer() {
  const { pause } = useIntervalFn(() => {
    if (activeTimeSession.value) {
      if (activeTimeSession.value.startedAt) {
        const nowDate = new Date();

        elapsedTime.value =
          (nowDate.getTime() - activeTimeSession.value.startedAt.getTime()) /
          1000;
      }
    }
  }, 1000);

  timerPause.value = pause;
}

onMounted(() => {
  startTimer();
});

watch(
  () => currentTimerState,
  async (newData) => {
    console.log("time data updated");

    if (newData.value === "paused") {
      if (timerPause.value) {
        timerPause.value();
      }
    } else if (newData.value === "running") {
      currentTimerState.value = store.getCurrentState;
      activeTimeSession.value = store.activeTimeSession;
    }
  },
);

async function stopTimer() {
  await store.stopActiveSession();
}
</script>

<style scoped></style>
